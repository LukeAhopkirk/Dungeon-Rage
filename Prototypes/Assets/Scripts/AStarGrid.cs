...
public class AStarGrid : MonoBehaviour {
	...
	/* Finds the node on the list with smallest cost+guess, removes it from
	 * the list and returns referece to it. 
	 * 
	 * param: alist - list of nodes
     * returns: AStarNode - node with smallest cost+guess
    */
	private AStarNode PopSmallestCostPlusGuessNode(List<AStarNode> alist)
	{
		// Get the first node on the list and set it as the curent
		// node to pop off the list
		AStarNode popNode = alist.First();
		float popTotal = popNode.cost + popNode.guess;
		int popIndex = 0;

		// Check other nodes on the list
		for (int nextIndex = 1; nextIndex < alist.Count; nextIndex++)
		{
			AStarNode nextNode = alist.ElementAt(nextIndex);
			float nextTotal = nextNode.cost + nextNode.guess;

			// If the cost+guess of the next node is less than the current
			// pop nodes, set it as the new pop node
			if (nextTotal < popTotal)
			{
				popNode = nextNode;
				popTotal = nextTotal;
				popIndex = nextIndex;
		...
		// If the node to pop is not last on the list, take the last item
		// from the list and put it in place of the pop node
		if (popIndex != alist.Count - 1)
		{
			alist[popIndex] = alist[alist.Count - 1];

		}
		// Remove the last item on the list
		alist.RemoveAt(alist.Count - 1);

		// Return the node that was poped
		return popNode;
	}
	...
	public List<Vector2> ShortestPath(Vector2 start, Vector2 target) {
		...
		//Go through all the enodes on open list
		while(open.Count > 0)
        {
			// Pop the next node to examine from the open list -
			// it's the node that has the smallest cost+guess
			AStarNode node = PopSmallestCostPlusGuessNode(open);

			// If the popped node is the target node, return
			// them found path
			if (node == targetNode)
			{
				return node.getPath();
			...
			//Condsider all the paths (edges) branching out of the
			//examined node...
			foreach (AStarEdge edge in node.neighbours)
			{
				//Get the newNode that is connected to this node
				AStarNode newNode = edge.getNeighbour(node);

				//Check if the new node is on closed list (meaning, wheather
				//it has been already examined)
				if (closed.Count > 0 && closed.Contains(newNode))
				{
					continue;
				}

				//Check if the new node is on open list (meaning, wheather
				//it is already waiting to be examined)
				if (open.Count > 0 && open.Contains(newNode))
				{
					continue;
				}


				//Compute the guess distance from the new node to the target
				//(as the crow flies)
				float newGuess = Vector2.Distance(newNode.position, targetNode.position);
				//Compute the cost from the start to the current node
				float newCost = node.cost + edge.cost;
				newNode.prev = node;
				newNode.cost = newCost;
				newNode.guess = newGuess;

				//Add the new node to the open list
				open.Add(newNode);
			}
			//Add the examined node to the closed list
			closed.Add(node);

		}
		// If this statement is reached, no path was found to the target
		return null;
	}
	...
	*/
	public void OnDrawGizmosSelected()
	{
		...
		foreach (AStarNode node in closed) {
